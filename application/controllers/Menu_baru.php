...
class Menu_baru extends CI_Controller
{
    ...
    private function _export_perkara_putus_harian($sheet, $data, $tanggal)
    {
        ...
        foreach ($data as $item) {
            ...
            $sheet->setCellValue('C' . $row, $item->jenis_perkara ?? '');
            $sheet->setCellValue('D' . $row, $item->tanggal_putus ?? '');
            $sheet->setCellValue('E' . $row, $item->hakim ?? '');
            $sheet->setCellValue('F' . $row, $item->status_bht ?? '');
            $row++;
        }
    }
    ...
    private function _export_jadwal_bht_harian($sheet, $data, $tanggal)
    {
        ...
        foreach ($data as $item) {
            ...
            $sheet->setCellValue('B' . $row, $item->nomor_perkara ?? '');
            $sheet->setCellValue('C' . $row, $item->jenis_perkara ?? '');
            $sheet->setCellValue('D' . $row, $item->target_bht ?? '');
    ...
    private function _export_perkara_tanpa_pbt($sheet, $data, $tanggal)
    {
        ...
        foreach ($data as $item) {
            ...
            $sheet->setCellValue('C' . $row, $item->jenis_perkara ?? '');
            $sheet->setCellValue('D' . $row, $item->tanggal_putus ?? '');
            $sheet->setCellValue('E' . $row, $item->hari_sejak_putus ?? '');
            $sheet->setCellValue('F' . $row, $item->level_peringatan ?? '');
            $row++;
        }
    }
}