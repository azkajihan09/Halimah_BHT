...
class Menu_baru_model extends CI_Model
{
	...
	public function get_jadwal_bht_harian($tanggal, $jenis = 'semua', $tahun_filter = '2025')
	{
		$this->db->select("
            ...
            CASE
                WHEN p.tahapan_terakhir_id >= 30 THEN 'Kasasi'
                WHEN p.tahapan_terakhir_id >= 25 THEN 'Banding'
                WHEN p.tahapan_terakhir_id >= 20 THEN 'Verzet'
                ELSE 'Normal'
            END as keterangan_perkara,
            ...
            CASE
                WHEN p.tahapan_terakhir_id >= 20 THEN 'Upaya Hukum'
                WHEN pb_pip.tanggal_transaksi IS NOT NULL AND pppp.tanggal_pemberitahuan_putusan IS NULL THEN 'TGL PIP belum ada'
                WHEN pb_pip.tanggal_transaksi IS NOT NULL AND pppp.tanggal_pemberitahuan_putusan IS NOT NULL THEN 
                    CASE 
                        WHEN pb_pip.tanggal_transaksi IS NULL THEN DATE_ADD(pp.tanggal_putusan, INTERVAL 15 DAY)
                        ELSE DATE_ADD(pppp.tanggal_pemberitahuan_putusan, INTERVAL 15 DAY)
                    END
            ...
            CASE
                WHEN pb_pip.tanggal_transaksi IS NOT NULL THEN pen.jurusita_text
                ELSE REPLACE(pen.jurusita_text, 'Juru Sita Pengganti', '')
            END as jsp,
	...
	public function get_perkara_putus_tanpa_pbt($tanggal)
	{
		...
		// Filter untuk tidak menampilkan perkara permohonan (mengandung /Pdt.P/)
		$this->db->where('p.nomor_perkara NOT LIKE "%/Pdt.P/%"');
