...
-- Command 1: Add jurusita column to berkas_masuk table
ALTER TABLE `berkas_masuk` 
ADD COLUMN `jurusita` varchar(255) NULL AFTER `panitera_pengganti`;

-- Command 2: Add jurusita column to berkas_pbt table  
ALTER TABLE `berkas_pbt` 
ADD COLUMN `jurusita` varchar(255) NULL AFTER `panitera_pengganti`;

-- Command 3: Update status enum values for berkas_masuk
-- First update existing data
UPDATE `berkas_masuk` SET `status_berkas` = 'PANITERA_PENGGANTI' WHERE `status_berkas` = 'MASUK';
UPDATE `berkas_masuk` SET `status_berkas` = 'BELUM_ADA_PBT' WHERE `status_berkas` = 'PROSES';  
UPDATE `berkas_masuk` SET `status_berkas` = 'SELESAI_ARSIP' WHERE `status_berkas` IN ('SELESAI', 'ARSIP');

-- Command 4: Modify enum column
ALTER TABLE `berkas_masuk` 
MODIFY COLUMN `status_berkas` enum(
    'PANITERA_PENGGANTI',
    'ALIH_MEDIA', 
...
DESCRIBE `berkas_masuk`;
DESCRIBE `berkas_pbt`;
SELECT status_berkas, COUNT(*) as jumlah FROM berkas_masuk GROUP BY status_berkas;
